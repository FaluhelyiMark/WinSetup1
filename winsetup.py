...
class WinSetup(tk.Tk):
    ...
    def make_toggle_row(self, parent, label, desc, tid, on_fn=None, off_fn=None, on_cmd=None, off_cmd=None, on_msg="Kész!", off_msg="Visszaállítva."):
        ...
        tk.Label(info, text=desc, font=("Consolas", 9), bg=SURFACE, fg=TEXT2, anchor="w", wraplength=490).pack(anchor="w")
    ...
    def build_privacy(self, parent):
        ...
        self.section_label(inner, "\U0001f5d1\ufe0f  Bloatware")
        self.make_button_row(inner, "\U0001f5d1\ufe0f  Bloatware eltávolítása", "Bing, Weather, Office Hub, Solitaire, People, Teams és más felesleges appok törlése", self.remove_bloatware)
        tk.Frame(inner, bg=BG, height=16).pack()

    def build_performance(self, parent):
        inner = self.make_scroll_frame(parent)
        self.section_label(inner, "⚡  Teljesítmény")
        self.make_toggle_row(inner, "🎯  Játék optimalizálás", "Game Mode bekapcsolása + nagy teljesítményű energiaséma", "gamemode",
    ...
    def build_appearance(self, parent):
        ...
        self.make_toggle_row(inner, "🔍  Keresősáv elrejtése", "Keresősáv eltüntetése a tálcáról (csak ikon marad)", "searchbar",
            ...
            on_msg="Keresősáv elrejtve! (Explorer újraindítás után látható)", off_msg="Keresősáv visszakapcsolva.")
        ...
        self.make_toggle_row(inner, "🗂️  Feladatnézet gomb elrejtése", "Feladatnézet (Task View) gomb eltüntetése a tálcáról", "taskview",
            ...
            on_msg="Feladatnézet gomb elrejtve!", off_msg="Feladatnézet gomb visszakapcsolva.")
    ...
    def remove_bloatware(self):
        self.log("⏳ Bloatware eltávolítása (ez eltarthat egy ideig)...")
        apps = [
            "Microsoft.BingNews",
        ...
        def task():
            ...
            for app in apps:
                ...
                if r.returncode == 0:
                    ...
                    failed += 1
            self.after(0, lambda: self.log(f"✅ Bloatware eltávolítva! ({removed} app törölve, {failed} nem volt telepítve)"))
        threading.Thread(target=task, daemon=True).start()
