...
class WinSetup(tk.Tk):
    ...
    def direct_install(self, url, filename, success_msg):
        self.after(0, lambda: self.log(f"{filename} letöltése..."))
        def task():
            try:
                dest = os.path.join(os.environ.get("TEMP", "C:\\Temp"), filename)
                subprocess.run(f'powershell -Command "Invoke-WebRequest -Uri '{url}' -OutFile '{dest}'"', shell=True, capture_output=True)
                subprocess.Popen(dest, shell=True)
                self.after(0, lambda: self.log(f"● {success_msg} (telepítő elindítva)"))
            except Exception as e:
